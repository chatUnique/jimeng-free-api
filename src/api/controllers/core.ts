...
export async function getCredit(refreshToken: string) {
  ...
  } = await user_credit_request("POST", "/commerce/v1/benefits/user_credit", refreshToken, {
    data: {},
    headers: {
...
export async function request(
  ...
  logger.info(options.responseType);
  logger.info(token);
  // 流式响应直接返回response
  if (options.responseType == "stream") return response;
  return checkResult(response);
}
export async function user_credit_request(
  method: string,
  uri: string,
  refreshToken: string,
  options: AxiosRequestConfig = {}
...
export async function token_request(
  method: string,
  uri: string,
  refreshToken: string,
  options: AxiosRequestConfig = {}
